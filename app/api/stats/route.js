...
export async function GET(request) {
  try {
    ...
    const [
      totalViews, uniqueVisitors, topReferrers, viewsOverTime,
      ...
      vslWatchtime, vslPlays, vslCompletes, vslPauses
    ] = await Promise.all([
      ...
      sql`SELECT event_data->'data'->>'utm_source' as source, event_data->'data'->>'utm_medium' as medium, event_data->'data'->>'utm_campaign' as campaign, COUNT(*) as count FROM events WHERE event_type = 'event' AND event_data->>'name' = 'utm' AND created_at >= ${since} GROUP BY source, medium, campaign ORDER BY count DESC LIMIT 10`,
      ...
      // VSL Video watchtime retention (each 10% milestone)
      ...
      // VSL pause positions (where people stop)
